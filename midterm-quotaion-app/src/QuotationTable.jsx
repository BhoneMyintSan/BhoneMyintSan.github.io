...
function QuotationTable({ data, clearDataItems, deleteByIndex }) {
  ...
  const distotal = data.reduce((acc, v) => acc + v.qty * v.discount,0)
  const clearTable = () => {
    clearDataItems();
  };
  ...
  const formatAmount = (qty, ppu, discount) => {
    const amount = qty * ppu;
    const finalAmount = amount - discount;
    return `${ppu} x ${qty} - ${discount} = ${finalAmount}`;
  };
      ...
      <Table striped bordered hover>
          ...
          data.map((v, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className={style.textRight}>{formatAmount(v.qty,v.ppu,v.discount)}</td>
              </tr>
            );
          })
        }</tbody>
        <tfoot>
          <tr>
            ...
            <td className={style.textRight}>{distotal}</td>
